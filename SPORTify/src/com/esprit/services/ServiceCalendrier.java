...
public class ServiceCalendrier implements IserviceCalendrier<Calendrier> {
    ...
    public List<Calendrier> afficherCalendrier() throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                list.add(new Calendrier(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4)));
            }
    ...
    @Override
    public List<Calendrier> rechercherCoursDansCalendrier() throws SQLException {
        List<Calendrier> list = new ArrayList<>();

        try {
            String requete = "SELECT * FROM calendrier where Type_Cours= 'aerobique' ";
            PreparedStatement pst = cnx.prepareStatement(requete);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                list.add(new Calendrier(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4)));
